...
object ErrorExercises {
  ...
  /**
    ...
    */
  def findPropertyAgent(propertyId: PropertyId) : ErrorOr[Agent] = findProperty(propertyId).map(_.agentId).flatMap(findAgent)
  ...
  /**
    ...
    */
  def findAgents(agentIds: Vector[AgentId]): Vector[ErrorOr[Agent]] = agentIds.map(findAgent)
  ...
  /**
    ...
    */
  def sequenceAgents(agents: Vector[ErrorOr[Agent]]): ErrorOr[Vector[Agent]] = agents.sequence[ErrorOr, Agent]
  ...
  def findAllAgents(agentIds: Vector[AgentId]): ErrorOr[Vector[Agent]] = agentIds.traverse[ErrorOr, Agent](findAgent)
  ...
  def checkAllAgents(agentIds: Vector[AgentId]): ErrorOr[Unit] = agentIds.traverseU_(findAgent)
  ...
  /** Exercise 8:
    ...
    */
  def findAllAgentsNames(agentIds: Vector[AgentId]): ErrorOr[Vector[String]] = findAllAgents(agentIds) match {
    case Right(x) => Right(x.map(_.name))
    case Left(e) => Left(e)
  }
  ...
  def suggestAProperty2(propertyId: PropertyId, agentId: AgentId): ErrorOr[String] = {
    Apply[ErrorOr].map2(findProperty(propertyId), findAgent(agentId))(
      (property, agent) => s"Hey ${agent.name} how about selling ${property.description}"
    )
  }
